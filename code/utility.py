...
class MLP( nn.Module ):
    '''
    Multi-layer perceptron class. It is a simple feed-forward neural network
    with 2 hidden layers of size 64 and ReLU activation functions.
    '''
...
class ReplayBuffer():
    '''
    Class for the replay buffer. It stores the samples in a tensor and provides methods 
    to easily update its content and generate random batches.
    '''
    ...
    def update(self, state, action, next_state, reward):
        '''
        Update the buffer with a new sample. If the buffer is full, the oldest sample is replaced.
        The mean and variance of the buffer are also updated (instead of re-computed).
        '''
        ...
        self.values = torch.roll( self.values, -1, dims=0 ) 
